universities=[{
    "recommendation": {},
    "name": "University of Dar es Salaam",
    "acronym":"UDSM",
    "reg_no":"001",
    "phoneNumber": "+255222410700",
    "coordinates": {
        "latitude": -6.780785,
        "longitude": 39.202735
    },
    "website":"http://www.udsm.ac.tz",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "region": "Dar es Salaam",
    "address":"Dar es Salaam",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "University of Dodoma",
    "acronym":"UDOM",
    "reg_no":"019",
    "phoneNumber": "+255262310003",
    "coordinates": {
        "latitude": -6.214177,
        "longitude": 35.824725
    },
    "website":"http://www.udom.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "region": "Dodoma",
    "address":"Kikuyu, Avenue",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "State University of Zanzibar",
    "acronym":"SUZA",
    "reg_no":"010",
    "phoneNumber": "+255242230724",
    "coordinates": {
        "latitude": -6.165802,
        "longitude": 39.191811
    },
    "website":"http://suza.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Vuga Rd, Zanzibar",
    "region": "	Zanzibar Urban",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Sokoine University of Agriculture",
    "acronym":"SUA",
    "reg_no":"014",
    "phoneNumber": "+255232603511",
    "coordinates": {
        "latitude": -6.852164,
        "longitude": 37.657590
    },
    "website":"http://www.sua.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Morogoro",
    "region": "	Morogoro",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 

{
    "recommendation": {},
    "name": "The Open University of Tanzania",
    "acronym":"OUT",
    "reg_no":"003",
    "phoneNumber": "+255232603511",
    "coordinates": {
        "latitude": -6.783552,
        "longitude": 39.264661
    },
    "website":"http://www.out.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Dar es Salaam, Tanzania",
    "region": "	Dar es Salaam",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Nelson Mandela African Institute of Science and Technology",
    "acronym":"NM–AIST",
    "reg_no":"016",
    "phoneNumber": "+255272970007",
    "coordinates": {
        "latitude": -3.399486,
        "longitude": 36.796808
    },
    "website":"http://www.nm-aist.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Arusha, Tanzania",
    "region": "Arusha",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Mbeya University of Science and Technology",
    "acronym":"MUST",
    "reg_no":"026",
    "phoneNumber": "+255252502861",
    "coordinates": {
        "latitude": -8.941853,
        "longitude": 33.416029
    },
    "website":"https://www.mustnet.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Chuo kikuu cha sayansi na teknolojia, Mbeya",
    "region": "Mbeya",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
},  
{
    "recommendation": {},
    "name": "Muhimbili University of Health and Allied Sciences",
    "acronym":"MUHAS",
    "reg_no":"015",
    "phoneNumber": "+255222151596",
    "coordinates": {
        "latitude": -6.807826,
        "longitude": 39.273270
    },
    "website":"https://www.muhas.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"United Nations Rd, Dar es Salaam",
    "region": "Dar es Salaam",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Ardhi University",
    "acronym":"ARU",
    "reg_no":"004",
    "phoneNumber": "+255222151596",
    "coordinates": {
        "latitude": -6.766172,
        "longitude": 39.215419
    },
    "website":"http://www.aru.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"University vimweta new building, Dar es Salaam",
    "region": "Dar es Salaam",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Moshi Co-operative University",
    "acronym":"MoCU",
    "reg_no":"027",
    "phoneNumber": "+255272754401",
    "coordinates": {
        "latitude": -3.336594,
        "longitude": 37.338602
    },
    "website":"http://www.mocu.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Sokoine Rd, Moshi",
    "region": "Kilimanjaro",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Mzumbe University",
    "acronym":"MU",
    "reg_no":"011",
    "phoneNumber": "+255232931220",
    "coordinates": {
        "latitude": -6.923931,
        "longitude": 37.569069
    },
    "website":"http://admission.mzumbe.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Magorofani Street,Morogoro",
    "region": "Morogoro",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Hubert Kairuki Memorial University",
    "acronym":"HKMU",
    "reg_no":"005",
    "phoneNumber": "+255713278060",
    "coordinates": {
        "latitude": -6.773070,
        "longitude": 39.254773
    },
    "website":"http://www.hkmu.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"New Hub St, Dar es Salaam",
    "region": "Dar es Salaam",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "International Medical and Technological Universiy",
    "acronym":"IMTU",
    "reg_no":"006",
    "phoneNumber": "+255743400011",
    "coordinates": {
        "latitude": -6.710777,
        "longitude": 39.211056
    },
    "website":"http://www.imtu.edu/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Bagamoyo Rd, Dar es Salaam",
    "region": "Dar es Salaam",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Tumaini University Makumira",
    "acronym":"TUMA",
    "reg_no":"007",
    "phoneNumber": "+255232931220",
    "coordinates": {
        "latitude": -3.371153,
        "longitude": 36.828455
    },
    "website":"http://www.makumira.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Arusha",
    "region": "Arusha",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "St. Augustine University of Tanzania",
    "acronym":"SAUT",
    "reg_no":"008",
    "phoneNumber": "+255736200943",
    "coordinates": {
        "latitude": -6.923931,
        "longitude": 37.569069
    },
    "website":"https://st-augustine-university-of-tanzania.business.site/?utm_source=gmb&utm_medium=referral",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"malimbe-Luchelele, Mwanza",
    "region": "Mwanza",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Zanzibar University",
    "acronym":"ZU",
    "reg_no":"009",
    "phoneNumber": "+255772601303",
    "coordinates": {
        "latitude": -6.224787,
        "longitude": 39.312229
    },
    "website":"http://www.zanvarsity.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Tunguu, Zanzibar",
    "region": "Zanzibar Urban",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Mount Meru University",
    "acronym":"MMU",
    "reg_no":"012",
    "phoneNumber": "+255272502412",
    "coordinates": {
        "latitude": -3.299985,
        "longitude": 36.633604
    },
    "website":"http://www.mmu.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"A 104,Arusha",
    "region": "Arusha",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "University of Arusha",
    "acronym":"UoA",
    "reg_no":"013",
    "phoneNumber": "+255744952773",
    "coordinates": {
        "latitude": -3.327210,
        "longitude": 36.880606
    },
    "website":"http://www.uoa.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Momela Rd, Arusha",
    "region": "Arusha",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Teofilo Kisanji University",
    "acronym":"TEKU",
    "reg_no":"014",
    "phoneNumber": "+255252502682",
    "coordinates": {
        "latitude": -8.919544,
        "longitude": 33.473264
    },
    "website":"http://teku.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Mbeya",
    "region": "Mbeya",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Muslim University of Morogoro",
    "acronym":"MUM",
    "reg_no":"017",
    "phoneNumber": "+255715202911",
    "coordinates": {
        "latitude": -6.801203,
        "longitude": 37.659169
    },
    "website":"http://www.mum.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Morogoro",
    "region": "Morogoro",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "St. John's University of Tanzania",
    "acronym":"SJUT",
    "reg_no":"018",
    "phoneNumber": "+255633280846",
    "coordinates": {
        "latitude": -6.198205,
        "longitude": 35.731452
    },
    "website":"http://www.sjut.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Kikuyu Ave, Dodoma",
    "region": "Dodoma",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "University of Bagamoyo",
    "acronym":"UoB",
    "reg_no":"020",
    "phoneNumber": "+255715673096",
    "coordinates": {
        "latitude": -6.708893,
        "longitude": 39.216370
    },
    "website":"http://uob.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"NSSF Rd,Dar es Salaam",
    "region": "Dar es Salaam",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Eckernforde Tanga University",
    "acronym":"ETU",
    "reg_no":"021",
    "phoneNumber": "+255272645936",
    "coordinates": {
        "latitude": -5.101534,
        "longitude": 39.045027
    },
    "website":"http://www.eckernfordetangauniversity.ac.tz",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"A14,Tanga",
    "region": "Tanga",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Catholic University of Health and Allied Sciences",
    "acronym":"CUHAS",
    "reg_no":"22",
    "phoneNumber": "+255282500881",
    "coordinates": {
        "latitude": -2.528967,
        "longitude": 32.908482
    },
    "website":"http://www.bugando.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Bugando Medical Center,Mwanza",
    "region": "Mwanza",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "St. Joseph University In Tanzania",
    "acronym":"SJUIT",
    "reg_no":"023",
    "phoneNumber": "+255673287723",
    "coordinates": {
        "latitude": -6.791932,
        "longitude": 39.088545
    },
    "website":"http://www.sjuit.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Brigitta Campus, Dar es Salaam",
    "region": "Dar es Salaam",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "United African University of Tanzania",
    "acronym":"UAUT",
    "reg_no":"024",
    "phoneNumber": "+255684505012",
    "coordinates": {
        "latitude": -6.870540,
        "longitude": 39.331133
    },
    "website":"https://www.uaut.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Kisiwani,Dar es Salaam",
    "region": "Dar es Salaam",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Sebastian Kolowa Memorial University",
    "acronym":"SEKOMU",
    "reg_no":"027",
    "phoneNumber": "+255272977003",
    "coordinates": {
        "latitude": -4.751147,
        "longitude": 38.297374
    },
    "website":"http://www.sekomu.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Tanga",
    "region": "Tanga",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "Kampala International University In Tanzania",
    "acronym":"KIUT",
    "reg_no":"028",
    "phoneNumber": "+255782777775",
    "coordinates": {
        "latitude": -6.886429,
        "longitude": 39.156108
    },
    "website":"http://www.kiut.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Pugu Road, Dar es Salaam",
    "region": "Dar es Salaam",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 
{
    "recommendation": {},
    "name": "University of Iringa",
    "acronym":"UoI",
    "reg_no":"030",
    "phoneNumber": "+255743802615",
    "coordinates": {
        "latitude": -7.741308,
        "longitude": 35.725215
    },
    "website":"http://www.uoi.ac.tz/",
    "open": true,
    "openStatusText": "Open until 9:00 PM",
    "address":"Iringa",
    "region": "Iringa",
    "schedule": [{
        "dayName": "Today",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tomorrow",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Monday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Tuesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Wednesday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Thursday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }, {
        "dayName": "Friday",
        "hours": "5:00 AM to 9:00 PM",
        "open": true,
        "holiday": ""
    }],
}, 

]